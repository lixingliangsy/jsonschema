...
export default function Home() {
  ...
  const price = (PRODUCT as any).priceMonthly ?? 19
  ...
  async function onSignup(e: FormEvent) {
    ...
    setSignupMsg('')
    try {
      const r = await fetch('/api/checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: value, plan: 'free' }) })
      const data = await r.json().catch(() => ({}))
      setSignupMsg(data?.error ? data.error : "You're in — opening the studio…")
  ...
  return (
    <>
      <Head>
        ...
        <link rel="canonical" href="https://jsonschema.lxsaihub.com/" />
      
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: `{"@context": "https://schema.org", "@type": "FAQPage", "mainEntity": [{"@type": "Question", "name": "What is JSONSchema?", "acceptedAnswer": {"@type": "Answer", "text": "JSONSchema is a tool that turns a sample JSON object into a strict JSON Schema with explanations and validation rules."}}, {"@type": "Question", "name": "What schema version does it use?", "acceptedAnswer": {"@type": "Answer", "text": "It produces JSON Schema draft 2020-12, the current standard most validators support."}}, {"@type": "Question", "name": "Does it mark required fields?", "acceptedAnswer": {"@type": "Answer", "text": "Yes. It marks required fields automatically from the sample."}}, {"@type": "Question", "name": "Does it explain the rules?", "acceptedAnswer": {"@type": "Answer", "text": "It explains each validation rule in plain language so you know what the API should enforce."}}, {"@type": "Question", "name": "Who should use it?", "acceptedAnswer": {"@type": "Answer", "text": "API developers who want a strict, documented contract from a sample object."}}, {"@type": "Question", "name": "Is it compliant with modern tooling?", "acceptedAnswer": {"@type": "Answer", "text": "Draft 2020-12 compliance means the schema works with current validators."}}]}` }} />
        </Head>
      ...
      <div className="min-h-screen bg-white text-slate-900">
        ...
        <section className="bg-slate-950 text-white py-10">
          <div className="max-w-6xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            {[['json-schema-rules@2026-07-19', 'Ruleset'], ['7', 'Compliance checks'], ['2020-12', 'Draft'], ['<30s', 'Time to schema']].map(([n, l]) => (
              <div key={l}>
                <div className="text-2xl md:text-3xl font-black text-indigo-300 break-all">{n}</div>
        ...
        <section id="definition" className="py-20 bg-slate-50">
          <div className="max-w-6xl mx-auto px-6">
            ...
            <h2 className="text-3xl md:text-4xl font-extrabold mb-4">JSONSchema - definition</h2>
...
<section id="features" className="py-20">
          <div className="max-w-6xl mx-auto px-6">
            <div className="text-center mb-12">
              ...
              <h2 className="text-3xl md:text-4xl font-extrabold mb-3">Schemas your API can actually enforce</h2>
            </div>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
              {featCards.map((f) => (
                <div key={f} className="rounded-2xl border border-slate-200 p-6 shadow-sm hover:-translate-y-1 transition bg-white">
                  ...
                  <p className="text-sm text-slate-600">A deterministic compliance screen catches missing types and bad formats before you ship.</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section id="how" className="py-20 bg-slate-50">
          <div className="max-w-6xl mx-auto px-6">
            <div className="text-center mb-12">
              <h2 className="text-3xl md:text-4xl font-extrabold">From sample to schema in 3 steps</h2>
            </div>
            <div className="grid md:grid-cols-3 gap-6">
              {[
                ['1', 'Paste', 'Drop in a sample JSON, pick strictness and purpose.'],
                ['2', 'Generate', 'Model writes a draft 2020-12 schema, then the compliance ruleset screens type/required/format/enum.'],
                ['3', 'Report', 'Export the schema + explanation with a runId.'],
              ].map(([n, t, d]) => (
          ...
          <div className="max-w-6xl mx-auto px-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <form onSubmit={runStudio} className="rounded-2xl border border-slate-200 p-6 bg-white shadow-sm">
                <h3 className="font-bold mb-4">Generate schema</h3>
                {inputsSchema.map((f) => (
                  <div key={f.key} className="mb-4">
                    ...
                    {f.type === 'select' ? (
                      ...
                      <textarea
                        className="w-full border border-slate-200 rounded-xl p-3 min-h-[130px] font-mono text-sm"
              ...
              <div className="rounded-2xl border border-slate-200 p-6 bg-white shadow-sm">
                ...
                {ruleHits.length ? (
                  <div className="mb-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
                    <div className="text-xs font-bold text-slate-700 mb-1">Compliance findings</div>
                ...
                <pre className="whitespace-pre-wrap text-sm bg-slate-50 border border-slate-200 rounded-xl p-4 min-h-[180px] font-mono">{result || 'Your output will appear here.'}</pre>
                <div className="flex flex-wrap gap-3 mt-4">
                  ...
                  <a href="/api/checkout" className="px-4 py-2 rounded-full bg-indigo-600 text-white font-semibold">
                    ${price}/mo — Upgrade
                  </a>
                </div>
              </div>
...
<section id="pricing" className="py-20">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <div className="rounded-2xl border border-slate-200 p-6">
                ...
                <ul className="space-y-2 text-sm text-slate-600 mb-6">
                  <li className="flex gap-2"><Check /> {pricing[0]?.desc || '10 runs / mo'}</li>
              ...
              <div className="rounded-2xl border-2 border-indigo-600 p-6 relative shadow-lg">
                ...
                <ul className="space-y-2 text-sm text-slate-600 mb-6">
                  ...
                  <li className="flex gap-2"><Check /> Audit log · copy as Markdown/JSON text · fair-use cost guard</li>
                  <li className="flex gap-2"><Check /> Priority support · Yearly ${priceYearly}</li>
                </ul>
                <a href="/api/checkout" className="block text-center w-full py-3 rounded-full bg-indigo-600 text-white font-bold">
                  Start free trial
                </a>
                <a href="/api/checkout?cycle=yearly" className="block text-center mt-3 text-sm font-semibold text-indigo-600">
                  Or pay yearly
                </a>
              </div>
              <div className="rounded-2xl border border-slate-200 p-6">
        ...
        <section id="faq" className="py-20 bg-slate-50">
          <div className="max-w-3xl mx-auto px-6">
            ...
            <div className="space-y-3">
              {[
                ['Is the schema valid out of the box?', 'It follows draft 2020-12 and is screened by a deterministic compliance ruleset. For production, validate it against your real data with a library like Ajv.'],
                ['Does it check type / required / format / enum?', 'Yes. The ruleset (json-schema-rules@2026-07-19) flags a missing root type, required keys not in properties, unknown formats, and duplicate enum values.'],
                ['Do I need my own OpenAI key?', 'No for Free/Pro. AI runs are included via our platform key (fair use). Enterprise can optionally bring your own key (BYOK) at /settings — the key stays server-side only.'],
                ['What is Fair Use / what if I hit the AI limit?', 'Pro includes about 300 workflow runs/month (fair use cost guard on gpt-4o-mini, plus a daily cap). If you hit the limit, Studio returns HTTP 429 — upgrade, wait for reset, enable Demo mode, or use Enterprise BYOK. We never silently return fake AI success.'],
                ['Is checkout secure?', 'Payments are processed by Waffo Pancake (merchant of record).'],
                ['Does this guarantee my API is validated?', 'No. It is decision-support: a model-assisted schema plus a deterministic compliance screen. Wire the exported schema into your API validator to actually enforce it.'],
              ].map(([q, a]) => (
        ...
        <section className="py-20">
          <div id="signup" className="max-w-3xl mx-auto px-6 text-center rounded-3xl bg-slate-950 text-white p-10">
            <h2 className="text-3xl font-extrabold mb-3">Ship a schema you can enforce</h2>
            <p className="text-slate-300 mb-6">Join developers using {PRODUCT.name}. Free to try — no card needed.</p>
              ...
              <div className="p-6">
                <div className="rounded-xl border border-slate-200 bg-slate-950 text-white p-5 min-h-[260px]">
                  ...
                  {demoSteps[demoStep]?.preview ? (
                    <pre className="text-xs bg-slate-900 border border-slate-700 rounded-lg p-3 overflow-auto max-h-36 text-emerald-300 whitespace-pre-wrap font-mono">
                      {demoSteps[demoStep].preview}
                    </pre>
                  ) : null}
...
function Check() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="text-emerald-500 shrink-0">
